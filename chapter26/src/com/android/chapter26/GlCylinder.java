	...
	private void generateData()
	{
		...
		for (int i = 0; i < slices; i++)
		{
			...
			for (int j = 0; j <= stacks; j++)
			{
				...
				if (normals)
				{
					Utils.setXYZn(normalCoords, 3 * 2 * j, height * cosAlpha1, height * sinAlpha1, base - top);
					Utils.setXYZn(normalCoords, 3 * 2 * j + 3, height * cosAlpha0, height * sinAlpha0, base - top);
				}
